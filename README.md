...
- **Tool-Use Readiness Report** — `agents-shipgate-reports/report.{md,json,sarif}`. Markdown for human release review, JSON for tools and coding agents (current schema [v0.12](docs/report-schema.v0.12.json); gating signal is `release_decision.decision`; v0.12 adds per-finding `agent_action` and top-level `agent_summary`), SARIF for GitHub code-scanning workflows.
...
- **[`docs/manifest-v0.1.json`](docs/manifest-v0.1.json)** + **[`docs/report-schema.v0.12.json`](docs/report-schema.v0.12.json)** — JSON Schemas for live editor validation (current; emitted reports carry `report_schema_version: "0.12"`). v0.12 adds the per-finding `agent_action` enum and the top-level `agent_summary` block. Read `release_decision.decision` for release gating in new consumers; read `agent_summary.first_recommended_action` for a deterministic next step.
...
- The manifest remains `version: "0.1"` so existing configs keep working. Current reports carry `report_schema_version: "0.12"` (additive over v0.11's source provenance fields, adding the per-finding `agent_action` enum and top-level `agent_summary` block) while preserving the stable payload contract documented in the report schema.
...
- [JSON report schema v0.12](docs/report-schema.v0.12.json)
