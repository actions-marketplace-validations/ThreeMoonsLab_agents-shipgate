    ...
    for tool_name in sorted(required_tools - approved_tools):
        ...
        findings.append(
            _evidence_finding_for_tool(
                ...
                title=f"{tool_name} lacks approved HITL trace evidence",
                evidence=evidence,
                recommendation=(
                    f"Add local approval trace evidence showing {tool_name} "
                    "was approved before the tool call, or change the validation "
                    "review posture."
                ),
            )
        )
...
def _override_reason_findings(context: ScanContext) -> list[Finding]:
    ...
    if not log_files:
        ...
        return [
            agent_finding(
                ...
                title="Override evidence has events without reasons",
                severity="high",
                category="evidence",
                evidence={
    ...
    return [
        agent_finding(
            check_id="SHIP-EVIDENCE-OVERRIDE-REASON-MISSING",
            title="Override reason evidence is missing",
            severity="high",
            category="evidence",
            evidence={
                "required": "override_reason_required",
    ...
    for tool in sorted(context.tools, key=lambda item: item.name):
        ...
        findings.append(
            tool_finding(
                ...
                title=f"{tool.name} lacks high-risk auto-approval exclusion evidence",
                severity="high",
                category="evidence",
                evidence={
...
def _promotion_criteria_finding(
    ...
    *,
    ...
    return agent_finding(
        ...
        title="HITL promotion criteria evidence is missing",
        severity="high",
        category="evidence",
        evidence={
