...
CHECK_METADATA: list[CheckMetadata] = [
    ...
    _meta(id="SHIP-EVIDENCE-APPROVAL-TRACE-MISSING", category="evidence", default_severity="high", description="HITL approval trace evidence is missing for an approval-required tool.", rationale="Limited automation review depends on local evidence that approval-controlled actions were approved before the tool call.", fires_when="validation.required_evidence.approval_trace_required is true and no local approval trace shows approved=true for an approval-required tool.", evidence_fields=["tool_name", "required", "reason", "trace_files", "approved_tools"], recommendation="Add local approval trace evidence or change the validation review posture."),
    _meta(id="SHIP-EVIDENCE-OVERRIDE-REASON-MISSING", category="evidence", default_severity="high", description="HITL override reason evidence is missing.", rationale="Override, bypass, and auto-approval events need reviewer-visible reasons for governance review.", fires_when="validation.required_evidence.override_reason_required is true and override logs are absent, empty, or contain events without non-empty reasons.", evidence_fields=["required", "reason", "override_log_files", "events_missing_reason"], recommendation="Record non-empty reasons for override, bypass, and auto-approval events."),
    _meta(id="SHIP-EVIDENCE-HIGH-RISK-EXCLUSION-MISSING", category="evidence", default_severity="high", description="High-risk auto-approval exclusion evidence is missing.", rationale="High-risk tools that already declare approval policy need separate local evidence that they are excluded from auto-approval review posture.", fires_when="validation.required_evidence.high_risk_auto_approval_exclusion_required is true and a high-risk tool with declared approval policy is not listed in high_risk_auto_approval_exclusions.", evidence_fields=["required", "reason", "risk_tags", "exclusion_files", "excluded_tools"], recommendation="Document high-risk approval-controlled tools in high_risk_auto_approval_exclusions with reasons."),
    _meta(id="SHIP-EVIDENCE-HITL-PROMOTION-CRITERIA-MISSING", category="evidence", default_severity="high", description="HITL promotion criteria evidence is missing.", rationale="A limited auto-approval review posture needs local criteria evidence; Shipgate only structures the missing evidence for reviewers.", fires_when="validation.target_review_posture is limited_auto_approval and promotion criteria are absent or the canonical required-evidence flags are not true in the manifest and criteria file.", evidence_fields=["target_review_posture", "reason", "criteria_files", "manifest_flags_missing", "criteria_flags_missing"], recommendation="Add local promotion criteria evidence documenting the review posture and required evidence flags."),
